...
//		println!("{} {}", bx, by);
	...
	fn generate_noise_deux(range: std::ops::RangeInclusive<i32>, cx: i32, cy: i32, seed: u32) -> [i32; 129*129] {
...
//		let die = Uniform::from(127..=129);
	...
	fn generate_new(cells: &mut [u8; 128*128], wx: i32, wy: i32) {
		let weights = Self::generate_noise_deux(-999..=999, wx, wy, 0);
		let colors = Self::generate_noise_deux(-999..=999, wx, wy, 1);
		let mut i = 0;
		for y in 0..128 {
		for x in 0..128 {
//			cells[i] = if (x - 64) * (x - 64) + (y - 64) * (y - 64) > 32 * 64 { 0x80 } else { 0x81 };
			let blockval = weights[y * 129 + x];
			cells[i] = if blockval > 0 {
				let mut val = colors[y * 129 + x];
				val /= 30000000;
				if val < -4 {
					val = -4;
				} else if val > 4 {
